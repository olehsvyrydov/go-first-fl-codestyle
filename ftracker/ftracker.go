...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	speed := (float64(action) * lenStep / mInKm) / duration
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	speed := ((float64(action) * lenStep / mInKm) / duration) * kmhInMsec
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(speed, 2)/heightMeter)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
